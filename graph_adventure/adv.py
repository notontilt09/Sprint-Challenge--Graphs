...
# build adjacency list with '?' for unknown values

# loop until graph has same length of number of rooms
# while len(graph) < len(world.rooms):

## grab the current room player is in
current_room = player.currentRoom.id
# if we don't have that room in our adjacency list, add it with '?' for all directions
...
# look at our graph for current room, and move to one of the unexplored rooms
for direction in graph[current_room]:
    # if the room is unexplored
    if graph[current_room][direction] == '?':
        # travel there, add direction to the traversal path, store the new room
        player.travel(direction)
        traversalPath.append(direction)
        new_room = player.currentRoom.id

        # if the room is unexplored, initialize it in our graph with '?'s for each exit
