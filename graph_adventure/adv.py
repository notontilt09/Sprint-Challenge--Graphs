...
"""
Randomly move around unexplored rooms until we've visited everything algorithm
"""
# This is a terrible solution, but it passes the tests.  Sometimes takes 50,000+ steps
...
"""
following hints from readme.  This algorithm traverses randomly until it reaches a dead end.
Then it performs BFS back to closest room with an unexplored exit.  This 2 step process continues until
all rooms have been visited.
"""
# flip function used to describe bi-directional relationships between vertices
def flip(dir):
    if dir == 'n':
        return 's'
    elif dir == 's':
        ...
        return 'can\'t flip that direction'

# testing adding dft_recursive as first part
def dft_recursive(graph, player, traversalPath, current_room):
    """
    trying to go in a random direction as deep as possible till
    we hit a dead end
    """
    # shuffle the exits of the current room
    directions = list(graph[current_room].keys())
    random.shuffle(directions)
    # loop through exits to find unexplored one
    for direction in directions:
        # if exit is unexplored
        if graph[current_room][direction] == '?':
            # travel in that direction
            player.travel(direction)
            # add direction to the traversal path
            traversalPath.append(direction)
            # grab the new room that the player is in
            new_room = player.currentRoom.id

            # if the new room isn't in our graph, initialize it with '?'s
            if new_room not in graph:
                graph[new_room] = {}
                for move in player.currentRoom.getExits():
                    graph[new_room][move] = '?'
            
            # update our entries in the graph to reflect the relationship
            # between the room we we just in, and the room we're in now
            graph[current_room][direction] = new_room
            graph[new_room][flip(direction)] = current_room
            # update current_room to be the room we moved to for the next loop
            current_room = new_room
            # if we moved, we don't need to loop over the previous room's
            # possible exits any more.  In fact, it will break our code if 
            # we don't break out of this for loop once we've moved
            break
    if '?' in graph[current_room].values():
        dft_recursive(graph, player, traversalPath, current_room)

# BFS algorithm, used here to find the shortest path to a room with an unexplored exit
...
# loop until graph has same length as number of rooms
while len(graph) < len(world.rooms):
    ## grab the current room player is in
    current_room = player.currentRoom.id

    # if we don't have that room in our graph, initialize it with '?' for all directions
    if current_room not in graph:
        graph[current_room] = {}
        for direction in player.currentRoom.getExits():
            graph[current_room][direction] = '?'
    
    # perform depth first traversal to get us to a dead end
    dft_recursive(graph, player, traversalPath, current_room)
        
    # the above will traverse until we're at a dead end, then we'll get stuck in an infinite loop if
    # we haven't visited all of the rooms, and we don't have a '?' direction to move in
    
    # find the path to the nearest room with an unexplored exit
    # shortest path to a node found with BFS defined above
    path = bfs_unexplored(graph, current_room)
    # if we've hit every room, the bfs won't return anything... so we need to make sure there's a path
        ...
        for room in path:
            ...
            for direction in graph[current_room]:
                if graph[current_room][direction] == room:
                    # add the direction to our traveralPath and move the player
                    traversalPath.append(direction)
                    player.travel(direction)
            # set current_room to where the player currently is.  Originally had this inside of the if statement, but
            # we don't want to update the current_room until after the for loop has finished.
            current_room = player.currentRoom.id
...
# append number of moves to history.txt
history = open('history.txt', 'a')
history.write(f'{len(traversalPath)}\n')
history.close()
